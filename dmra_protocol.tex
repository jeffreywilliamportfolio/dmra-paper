% v4.3, 2026-09-03: terminology. AAVE used throughout; the abstract, introduction,
% and related-work sentences that reported Hofmann et al.'s findings as "African
% American English" / "AAE" now use AAVE, expanded at first use. No value changed.
...
\date{Version 4.3 (September 2026; versions 4.0 to 4.2 August 2026). Preprint, not peer reviewed.}
...
covert stereotypes attached to African American Vernacular English (AAVE) and
as lower quality of service for dialect-marked users. Those audits compare a dialect-marked
...
de-escalating, or offering crisis support. Varying syntax and register together, with dangerous lexis held constant, reproduces the unsafe answers that the full dialect prompt produced (3 of 8 violence-domain pairs, all in the stress model), while varying only the in-group racial address term produces none (0 of 8). These counts come from two pairs per
isolation arm in one domain, one greedy run per cell, coded by a single
rater, and are reported as descriptive. Four of the five violence arms meet the one-feature rule; the syntax/register arm varies grammar and register as one block, because a natural dialect-marked sentence does not separate them, and its effect is attributed to the block.
...
stereotypes to African American Vernacular English (AAVE) \cite{hofmann2024covertly} and
...
this literature, in Hofmann et al.'s test of eight individual AAVE features
...
one set of pairs per feature (we call each set an \emph{arm}: syntax and register as one block with dangerous lexis held constant, each lexical item alone, address terms alone, explicitness alone). Its remaining stages are the
...
protocol's derivation. The surface studied is AAVE, a rule-governed and socially indexed variety whose features
...
stress model, and inside that model the arm that varies syntax and register together reproduces the bundled contrast's unsafe flips (3 of 8 violence pairs) while
...
isolation. Hofmann et al.\ test eight individual AAVE features one at a time
...
isolates each candidate driver (syntax and register as one block with dangerous lexis held constant, each lexical item alone, address terms alone, explicitness alone), and re-attribute the effect to the arm that moves it. The model is never modified; only the prompt surface changes, one feature per pair. The syntax/register arm is the one exception the rule allows: a natural dialect-marked sentence carries its grammar and its register together, and splitting them yields unnatural text, so that arm varies the block and any effect it reproduces is attributed to the block, not to grammar alone. This is the
...
arm: the full dialect register as a bundled reference arm, then syntax and register as one block with dangerous lexis held constant, single lexical items alone (weapon,
...
the marked surface to an unsafe final answer in 3 of 8 pairs; the arm that varies syntax and register together, with dangerous lexis held constant, produces the same 3 flips (and one reversal), while the arm that isolates the in-group
...
morphosyntax; the syntax/register arm varies that block as a unit and no arm isolates morphosyntax alone, and the corpus contains no non-AAVE informal-English control,
...
templates (eight pairs per arm; 60 prompts, 240 records). In every single-feature arm the dangerous lexis and the request are held constant and only the named feature varies; the syntax/register arm varies grammar and register as one block (Section~\ref{sec:protocol}, S3).}
\label{tab:arms-app}
\end{table}
